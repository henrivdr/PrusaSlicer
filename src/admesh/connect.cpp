...
void stl_check_facets_nearby(stl_file *stl, float tolerance)
{
	...
	assert(uint32_t(stl->stats.connected_facets_1_edge) <= stl->stats.number_of_facets);

  	if (uint32_t(stl->stats.connected_facets_3_edge) == stl->stats.number_of_facets)
		  	...
		  	for (int i = 0; i < 3; ++ i)
		    	if (neighbors.neighbor[i] != -1) {
			    	int &other_face_idx = stl->neighbors_start[neighbors.neighbor[i]].neighbor[(neighbors.which_vertex_not[i] + 1) % 3];
			  		if ((uint32_t)other_face_idx != stl->stats.number_of_facets) {
